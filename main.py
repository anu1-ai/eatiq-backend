...
class ParseReceiptImageRequest(BaseModel):
    ...
    profile_country: str = "United Kingdom"  # used as currency fallback
...
CATEGORY_RULES = {
    ...
    "soft_drinks":["juice", "jce", "smoothie", "water", "sparkling",
                   ...
                   "squash", "cordial", "tea", "coffee", "cocoa",
                   "hot chocolate", "innocent", "tropicana", "oasis",
...
# ══════════════════════════════════════════════════════════════════════════════
# VISION PROMPT — enriched with real supermarket abbreviation examples
# ══════════════════════════════════════════════════════════════════════════════
VISION_PROMPT = """You are reading a grocery store receipt image for the EatIQ health app.
EatIQ works with any supermarket or grocery store worldwide.

Your ONLY job is to extract the raw text from this receipt exactly as printed.
Copy every character exactly — do NOT interpret, expand, or change anything.
Do NOT add items that are not on the receipt.
Do NOT reorder lines.
Each receipt line must be a separate string in raw_lines.

HOW TO READ A RECEIPT (universal rules):
- Item name is on the LEFT side of the line
- Price is on the RIGHT side of the same line
- Price may use any currency: £ $ € ¥ ₹ AED CHF kr RM SR or no symbol at all
- Decimal separator may be a period (1.49) OR a comma (1,49) — copy exactly as printed
- Discount/saving lines show a negative amount: -£0.55 or -0,45 or "Rabatt -1,20"
- Payment lines (Visa, contactless, Debit) — still include them
- If text is cut off at the edge, copy what is visible

STORE ABBREVIATION EXAMPLES (copy exactly as printed — do not expand):
  UK: JS, TTD, OEP, VV, ROWN, MR KIP, CAD, WALK, P.E, TF, TE, WR
  Germany: REWE, EDK, BIO (organic), PENNY
  France: CAR (Carrefour), ED, PRIX (price label)
  Netherlands: AH, JMB (Jumbo)
  US: GV (Great Value/Walmart), 365 (Whole Foods), TJ (Trader Joe's)
  Australia: WOW (Woolworths), COL (Coles)
  Loyalty discounts: "Nectar Price Saving", "Clubcard Price", "Payback",
    "Remise", "Korting", "Rabatt", "Descuento", "Bonuspunten"

Return this exact JSON:
{
  "store": "store name from receipt header",
  "date": "date printed on receipt, or empty string",
  "currency": "symbol as printed e.g. £ or € or $ or AED",
  "raw_lines": [
    "LINE ONE EXACTLY AS PRINTED",
    "LINE TWO EXACTLY AS PRINTED"
  ]
}
...
class Processor:
    def __init__(self):
        ...
        # ── Nutrition cache ──────────────────────────────────────────────────
        # Key: normalised item name (lowercase, stripped)
        # Value: nutrition dict from Open Food Facts
        # Lives in memory — resets on redeploy, rebuilds automatically.
        # At 10,000 entries ≈ 2MB RAM. No infrastructure needed.
        self._nutrition_cache: Dict[str, Dict] = {}
        self._cache_hits   = 0
        self._cache_misses = 0

    def _cache_key(self, term: str) -> str:
        """Normalise term to a stable cache key."""
    ...
    def fetch_nutrition(self, term: str, weight_str: str = None) -> Dict:
        ...
        if key in self._nutrition_cache:
            self._cache_hits += 1
            logger.info(f"Cache HIT [{self._cache_hits} hits / {self._cache_misses} misses]: '{term}'")
            return self._nutrition_cache[key]

        # ── Cache miss — call Open Food Facts ────────────────────────────────
        self._cache_misses += 1
        logger.info(f"Cache MISS [{self._cache_hits} hits / {self._cache_misses} misses]: '{term}'")
        try:
            ...
            if not hits:
                # Cache the empty result too — avoids repeated failed lookups
                self._nutrition_cache[key] = {}
                return {}
            best = hits[0]
            ...
            logger.error(f"OFF error for '{term}': {e}")
            return {}
        logger.info("Pre-processing receipt image")
        b64 = preprocess_receipt_image(b64)
        mime = "image/jpeg"  # always JPEG after pre-processing
                    ...
                    "content": [
                        {
                            ...
                            "image_url": {
                                "url": data_url,
                                "detail": "auto"  # auto = smarter than low, cheaper than high
                            }
                        },
                        {
                            "type": "text",
    ...
    def process_image(self, b64: str, mime: str, profile_country: str = "United Kingdom") -> Dict:
        extracted = self.extract_text_from_image(b64, mime)
        store     = extracted.get("store", "Supermarket")
        date      = extracted.get("date", "")
        # Use detected currency; fall back to profile country's currency
        ...
        return {
            "store": store, "date": date, "currency": currency,
