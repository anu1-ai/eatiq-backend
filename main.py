...
app = FastAPI(title="EatIQ Receipt Nutrition Proxy", version="2.1")
...
# ══════════════════════════════════════════════════════════════════════════════
STORE_PREFIXES = {
    "JS": "Sainsbury's", "TTD": "Taste the Difference", "OEP": "Organic",
    "SSTC": "Sainsbury's So Tasty", "SO ORG": "Sainsbury's Organic",
    "TF": "Tesco Finest", "TE": "Tesco", "TOS": "Tesco Organic", "TESCO": "Tesco",
    "WR": "Waitrose", "ESSE": "Essential Waitrose", "WTRSE": "Waitrose",
    "MS": "M&S", "MAS": "M&S", "MO": "Morrisons", "MORR": "Morrisons",
    "CO": "Co-op", "ASDA": "Asda", "AGF": "Asda Good & Balanced", "ICE": "Iceland",
}
...
def extract_price(line: str) -> Optional[float]:
    # Must be a positive price — not a discount line
    if re.search(r'-\s*£\d+\.\d{2}', line):
        return None
    match = re.search(r'(?<!-)\£(\d+\.\d{2})', line)
    if match:
        return float(match.group(1))
    # Bare decimal at end of line (some receipts omit £ symbol)
    match = re.search(r'(?<!\d)(\d{1,2}\.\d{2})\s*$', line)
    if match:
        val = float(match.group(1))
        if 0.10 <= val <= 200.0:  # sanity check — realistic grocery price
            return val
    return None

def extract_discount(line: str) -> Optional[float]:
    """Extract discount value from a saving/promo line. Returns positive float."""
    line_lower = line.lower()
    is_saving = any(re.search(p, line_lower) for p in [
        r"nectar\s+price\s+saving", r"nectar\s+saving", r"clubcard\s+price",
        r"clubcard\s+saving", r"price\s+saving", r"saving\s+£",
        r"you\s+saved", r"discount", r"promo", r"offer\s+saving",
        r"multibuy\s+saving", r"member\s+price", r"loyalty\s+saving",
    ])
    if not is_saving:
        return None
    # Match -£X.XX or £X.XX on a saving line
...
Each receipt line must be output as its own string in raw_lines.

Return a JSON object:
{
  "store": "store name from receipt header",
  "date": "date if visible on receipt, else empty string",
  "raw_lines": [
    "line 1 exactly as printed",
    "line 2 exactly as printed"
  ]
}

Include every single line — prices, discounts, totals, and all.
The downstream parser handles filtering. Your job is faithful extraction only.
...
class Processor:
    def __init__(self):
        ...
        self.off_headers = {"User-Agent": "EatIQ/2.1 (https://eatiq.app)"}
